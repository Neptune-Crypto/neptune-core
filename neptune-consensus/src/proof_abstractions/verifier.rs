...
/// if so returns true early (*i.e.*, without running the verifier). When the
/// test flag is set and the cache does not contain the claim and verification
/// succeeds, the claim is added to the cache. The only other way to populate
/// the cache is through method `cache_true_claim`.
