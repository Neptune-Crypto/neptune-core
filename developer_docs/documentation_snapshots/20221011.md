...
- rpc server to main: `mpsc`: Used to e.g. send a transaction object that is built from client-controlled UTXOs to the main thread where it can be added to the mempool. This channel is also used to shut down the program when the `shutdown` CLI command is called.
...
## Spending UTXOs
A transaction that spends UTXOs managed by the client can be made by calling the `create_transaction` method on the `GlobalState` instance. This function needs an synced `wallet_db` and a chain tip in `light_state` to produce a valid transaction.
...
XDG_DATA_HOME=~/.local/share/neptune-integration-test/0/ RUST_LOG=trace cargo run --bin rpc_client -- --server-addr 127.0.0.1:9799 send '[{"public_key": "0399bb06fa556962201e1647a7c5b231af6ff6dd6d1c1a8599309caa126526422e", "amount": 11}]' # Window2 RPC-client
```
