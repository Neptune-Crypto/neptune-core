...
For full rescanning functionality, the client must be started with the `--utxo-index` CLI flag. With this flag set, all transaction inputs (spending of UTXOs) is registered with a digest (40 bytes), and all announcements is registered with two b-field elements (a total of 16 bytes). So maintaining this UTXO index adds a bit to the size of the databases managed by the client.
