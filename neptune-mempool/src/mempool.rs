...
fn new_tx_wins_by_quality(
    ...
    match new_tier {
        ...
        ContestTier::SingleProof => conflicts.iter().any(|(conflicting_txkid, conflicting_tx)| {
            conflicting_tx.contest_tier() != ContestTier::SingleProof
    ...
    pub fn update_with_block(
        ...
        for (tx_id, tx) in &self.tx_dictionary {
            ...
            let AnyTx::Standard(transaction) = &tx.transaction else {
                // No update path for link transactions yet.
                continue;
            };
...
impl Mempool {
    ...
    /// The link transaction with this id, if any.
    ...
    mod link_tx_tests {
        ...
        fn inserted_link_is_held_but_invisible_to_standard_getters() {
            ...
            let [tx] = make_plenty_mock_transaction_supported_by_invalid_single_proofs(1)
                .try_into()
                .unwrap();
            let link = proof_backed_link(tx.kernel.clone(), arbitrary_addition_records(1));
            let txid = link.txid();
        ...
        fn cross_pipeline_conflict_is_decided_by_fee_density() {
            ...
            let [tx] = make_plenty_mock_transaction_supported_by_invalid_single_proofs(1)
                .try_into()
                .unwrap();
            let resident_kernel = TransactionKernelModifier::default()
                .mutator_set_hash(tip_msh)
                .fee(NativeCurrencyAmount::coins(5))
                .modify(tx.kernel);
            let resident = Transaction {
                kernel: resident_kernel.clone(),
                proof: tx.proof,
            };
            let resident_txid = resident.txid();
            mempool.insert(resident, UpgradePriority::Irrelevant);
            ...
            assert!(mempool.get(resident_txid).is_some());

            let rich = proof_backed_link(
                link_kernel(NativeCurrencyAmount::coins(50)),
                arbitrary_addition_records(1),
            );
            ...
            assert!(!mempool.contains(resident_txid));
        }
        ...
        fn link_with_mined_thruput_is_evicted() {
            ...
            let [tx_a, tx_b] = make_plenty_mock_transaction_supported_by_invalid_single_proofs(2)
                .try_into()
                .unwrap();
