...
RUST_BACKTRACE=1 XDG_DATA_HOME=~/.local/share/neptune-integration-test/0/ nice -n 1 --  cargo run -- --network regtest --peer-port 29790 --rpc-port 19790 --compose --guess -m 10 2>&1 | tee integration_test.log | sed 's/(.*)/\0 \[I0\]/g'  &
pid[0]=$!
sleep 5s;
