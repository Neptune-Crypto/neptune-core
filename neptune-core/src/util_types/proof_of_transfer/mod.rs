...
use tasm_lib::prelude::{Digest, Library, TasmObject, TasmStruct};
...
#[derive(TasmObject, BFieldCodec, Debug)]
pub(super) struct ProofOfTransferWitness {
    /// AOCL accumulator of the block
    aocl: MmrAccumulator,
    aocl_membership_proof: MmrMembershipProof,
...
mod tests {
    ...
    pub async fn setup_funded_wallet_with_mined_tx(
        ...
        gs_lock
            .set_new_self_composed_tip(block_1.clone(), vec![])
            .await
            .unwrap();
        ...
        gs_lock
            .record_own_transaction(&TxCreationArtifacts {
        ...
        gs_lock
            .set_new_self_composed_tip(block_with_tx.clone(), vec![])
            .await
            .unwrap();
        ...
        let target_addition_record =
            tx.details().tx_outputs.deref()[output_index].addition_record();
        ...
        gs_lock
            .set_new_self_composed_tip(block_after, vec![])
            .await
            .unwrap();
    ...
    mod thetritonprogram {
        ...
        async fn property_test_happy_path(
            ...
            let (_gs_lock, _block_after_digest, sent, claim) =
        ...
        async fn aocl_proof_verification_failed(
            ...
            let (_gs_lock, _block_after_digest, mut sent, _claim) =
                setup_funded_wallet_with_mined_tx(spend_percent, fee_percent, rness).await;
    ...
    mod helper_tests {
        ...
        async fn test_helper_nonexistent_block() {
            ...
            assert!(
                result.is_err(),
                "helper should fail with non-existent block"
            );

            let error_msg = result.unwrap_err().to_string();
            assert!(
                error_msg.contains("no canonical block"),
                "error should mention non-existent block: {}",
                error_msg
            );
        }
        ...
        async fn test_helper_function_evidence_canonical() {
            ...
            match result {
                Ok((claim, proof)) => {
                    ...
                    println!(
                        "   - Helper function succeeds with claim input length: {}",
                        claim.input.len()
                    );
                    println!("   - Proof data length: {}", proof_data.len());
                ...
                Err(e) => {
                    ...
                    if error_msg.contains("channel closed")
                        || error_msg.contains("channel recv error")
                    {
                        println!(
                            "⚠️  Channel issue in test environment (acceptable): {}",
                            error_msg
                        );
        ...
        async fn test_helper_invalid_tx_index() {
            ...
            let error_msg = result.unwrap_err().to_string();
            assert!(
                error_msg.contains("sent_transactions") || error_msg.contains("out of bounds"),
                "error should mention transaction index issue: {}",
                error_msg
            );
        }
        ...
        async fn test_helper_invalid_utxo_index() {
            ...
            assert!(
                result.is_err(),
                "helper should fail with invalid UTXO index"
            );
        }
    }
}
