...
mod tests {
    ...
    async fn property_test_happy_path(
        ...
        let (sent, claim) = setup_funded_wallet_with_mined_tx(spend_percent, fee_percent, rness).await;
    ...
    async fn aocl_proof_verification_failed(
        ...
        let (mut sent, _claim) = setup_funded_wallet_with_mined_tx(spend_percent, fee_percent, rness).await;
