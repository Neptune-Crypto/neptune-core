...
fn hard_fork_alpha() {
    ...
    async fn new_blocks_hardfork_alpha(block_primitive_witness: BlockPrimitiveWitness) {
        ...
        let mut bob =
            mock_genesis_global_state_with_block(2, bob_wallet, cli.clone(), block_a.clone()).await;
...
fn hard_fork_beta() {
    ...
    async fn new_blocks_hardfork_beta(block_primitive_witness: BlockPrimitiveWitness) {
        ...
        let mut bob =
            mock_genesis_global_state_with_block(2, bob_wallet, cli.clone(), minus3.clone()).await;
...
fn hard_fork_gamma() {
    ...
    async fn new_blocks_hardfork_gamma(block_primitive_witness: BlockPrimitiveWitness) {
        ...
        assert!(!bob.lock_guard().await.mempool.is_empty());
        let lustration_status_before = bob.lock_guard().await.chain.lustration_status().unwrap();
        bob.set_new_tip(hf.clone()).await.unwrap();
        assert!(
            bob.lock_guard().await.mempool.is_empty(),
            "Activating hardfork must clear mempool."
