...
pub async fn initialize(cli_args: cli_args::Args) -> Result<MainLoopHandler> {
    ...
    let rpc_service = application::rpc::server::NeptuneRPCServer::new(
    ...
    let tarpc_port = global_state_lock.cli().rpc_port;

    // start RPC servers: rest, jsonrpc, tarpc
    // these all share the same RPC service (endpoints)
    //
    // todo: make ports configurable
    let server_runner = multi_rpc::builder::ServerBuilder::new(rpc_service)
        .add_protocol(rpc::rest_axum(([127, 0, 0, 1], 9790).into()))
        .add_protocol(rpc::jsonrpsee(([127, 0, 0, 1], 9791).into()))
        .add_protocol(rpc::tarpc_tcp(([127, 0, 0, 1], tarpc_port).into()))
        .build()?;
