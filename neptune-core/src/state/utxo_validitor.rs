...
impl<'a> UtxoValidator<'a> {
    ...
    pub(crate) async fn mutxo_state(&self, monitored_utxo: &MonitoredUtxo) -> MonitoredUtxoState {
        ...
        // Saying "I don't know" or being wrong is okay as it only affects views
        // of spending history -- not spendable UTXOs.
        ...
        match self {
            ...
            UtxoValidator::Archival(archival_state) => {
                ...
                if is_spent {
                    // *Do not* set spend height to known here since you cannot
                    // trust the wallet database for this value when there are
                    // reorganizations.
                    MonitoredUtxoState::Spent(None)
                } else {
                    MonitoredUtxoState::SyncedAndUnspent
                }
            }
