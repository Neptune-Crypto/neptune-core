...
pub(super) struct WalletDbTables {
    ...
    /// The total number of output UTXOs generated by this wallet.
    // table number: 4
    pub(super) counter: DbtSingleton<u64>,

    /// The total number of generation keys generated by this wallet.
    //
    // This value coincides with the derivation index of the next key to be
    // generated.
    //
    // table number: 5
    pub(super) generation_key_counter: DbtSingleton<u64>,

    /// The total number of symmetric keys generated by this wallet.
    //
    // This value coincides with the derivation index of the next key to be
    // generated.
    //
    ...
    /// The total number of Elliptic curve hybrid keys generated by this wallet.
    // table number 14
    // Added after definition of wallet DB version 3 but no migration is
    // needed since if it's not initialized in the database, this value will
    // be set to zero. Which is exactly what a migration would need to do
    // anyway.
    pub(super) ech_key_counter: DbtSingleton<u64>,

    /// Total number of viewing address keys generated by this wallet.
    // table number 15
    pub(super) viewing_address_key_counter: DbtSingleton<u64>,
}
...
impl WalletDbTables {
    ...
    pub(super) async fn load_schema_in_order(storage: &mut SimpleRustyStorage) -> WalletDbTables {
        ...
        let ech_key_counter = storage.schema.new_singleton::<u64>("ech_key_counter").await;

        let viewing_address_key_counter = storage
            .schema
            .new_singleton::<u64>("viewing_address_key_counter")
            .await;
