...
pub struct WalletState {
    ...
    pub(crate) mempool_outgoing_utxos:
        HashMap<TransactionKernelId, HashMap<AbsoluteIndexSet, (Utxo, u64, u64)>>,
    mempool_incoming_utxos: HashMap<TransactionKernelId, Vec<IncomingUtxo>>,

    /// Cache of generation keys generated by this wallet.
    known_generation_keys: Vec<SpendingKey>,

    /// Cache of symmetric keys generated by this wallet.
    known_symmetric_keys: Vec<SpendingKey>,

    /// Cache of elliptic curve hybrid keys generated by this wallet.
    known_ec_hybrid_keys: Vec<SpendingKey>,

    /// Cache of known secret address keys generated by this wallet.
    known_secret_address_keys: Vec<SpendingKey>,
...
impl WalletState {
    ...
    pub(crate) async fn try_new(
        ...
        let known_ec_hybrid_keys = (0..rusty_wallet_database.get_ec_hybrid_key_counter())
            .map(|idx| wallet_entropy.nth_ec_hybrid_key(idx).into())
            .collect_vec();

        let known_secret_address_keys = (0..rusty_wallet_database.get_secret_address_key_counter())
            .map(|idx| wallet_entropy.nth_secret_address_key(idx).into())
            .collect_vec();

        let mut wallet_state = Self {
            wallet_db: rusty_wallet_database,
            wallet_entropy,
            mempool_outgoing_utxos: Default::default(),
            mempool_incoming_utxos: Default::default(),
            known_generation_keys,
            known_symmetric_keys,
            configuration: configuration.clone(),
            known_ec_hybrid_keys,
            known_secret_address_keys,
        };
        ...
        for key_type in KeyType::iter() {
            if wallet_state
                ...
                .is_none()
            {
                debug!("Adding 1st key for keytype: {key_type:?}");
                let _ = wallet_state.next_unused_spending_key(key_type).await;
            }
        }
    ...
    pub(in crate::state) async fn handle_mempool_event(&mut self, event: MempoolEvent) {
        match event {
            MempoolEvent::AddTx(tx_kernel) => {
                ...
                self.mempool_outgoing_utxos.insert(tx_id, spent_utxos);
                self.mempool_incoming_utxos.insert(tx_id, own_utxos);
            }
            MempoolEvent::RemoveTx(tx_kernel) => {
                let tx_id = tx_kernel.txid();
                debug!("handling mempool RemoveTx event.  tx: {}", tx_id);
                self.mempool_outgoing_utxos.remove(&tx_id);
                self.mempool_incoming_utxos.remove(&tx_id);
            }
        }
    }

    /// Get an iterator over (utxo, aocl_leaf_index) pairs of own inputs into
    /// transactions that live in the mempool.
    pub fn mempool_outgoing_utxos_iter(&self) -> impl Iterator<Item = (&Utxo, &u64)> {
        self.mempool_outgoing_utxos
            .values()
            .flatten()
            .map(|(_, (utxo, _mudbi, ali))| (utxo, ali))
    }

    /// Get an iterator over (utxo, addition_record) pairs corresponding to
    /// outputs of transactions that live in the mempool.
    pub fn mempool_incoming_utxos_iter(&self) -> impl Iterator<Item = (&Utxo, Digest, Digest)> {
            ...
            .map(|(txkid, sender_data)| {
                (
                    *txkid,
                    sender_data
                        .values()
                        .map(|(utxo, __, _)| utxo.get_native_currency_amount())
                        .sum::<NativeCurrencyAmount>(),
                )
            });

        let incoming = self
            .mempool_incoming_utxos
            .iter()
    ...
    /// Return a hashmap of UTXOs spent by this wallet in the transaction.
    ///
    /// The value type of this hash map is (UTXO, monitored-UTXO's list-index,
    /// AOCL leaf index).
    async fn scan_for_spent_utxos(
        &self,
        transaction_kernel: &TransactionKernel,
    ) -> HashMap<AbsoluteIndexSet, (Utxo, u64, u64)> {
            ...
            .map(|rr| &rr.absolute_indices)
        {
            ...
            {
                spent_own_utxos.insert(
                    index_set.to_owned(),
                    (mutxo.utxo, mutxo_list_index, mutxo.aocl_leaf_index),
                );
            }
        }
    ...
    /// Return an iterator over all `SpendingKey`s the wallet knows about.
    ///
    ...
    pub fn get_all_known_spending_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        KeyType::iter().flat_map(|key_type| self.get_known_spending_keys(key_type))
    }

    /// Return an iterator over all addressable `SpendingKey`s the wallet knows
    /// about.
    ///
    /// In particular, this includes all keys generated deterministically from
    /// the counters.
    pub fn get_all_known_addressable_spending_keys(
        ...
        KeyType::iter().flat_map(|key_type| self.get_known_addressable_spending_keys(key_type))
    }
    ...
    fn get_future_spending_keys_internal(
        ...
        match key_type {
            ...
            KeyType::EcHybrid => Box::new(
                self.get_future_ec_hybrid_keys(num_future_keys)
                    .map(|(i, key)| (i, SpendingKey::from(key))),
            ),
            KeyType::SecretAddress => Box::new(
                self.get_future_secret_address_keys(num_future_keys)
                    .map(|(i, key)| (i, SpendingKey::from(key))),
            ),
        }
    }
    ...
    pub fn get_known_spending_keys(
        ...
        match key_type {
            ...
            KeyType::SecretAddress => Box::new(self.get_known_secret_address_keys()),
        }
    }

    /// Return an iterator over all addressable `SpendingKey`s of the given type
    /// that the wallet knows about.
    ///
    /// In particular, this includes all keys generated deterministically from
    /// the counter.
    ...
    /// An iterator over all known symmetric spending keys.
    fn get_known_symmetric_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        self.known_symmetric_keys.iter().cloned()
    }

    /// An iterator over all known elliptic curve hybrid keys.
    fn get_known_ec_hybrid_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        self.known_ec_hybrid_keys.iter().cloned()
    }

    /// An iterator over all known elliptic curve hybrid keys.
    fn get_known_secret_address_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        self.known_secret_address_keys.iter().cloned()
    }

    /// Get the next unused spending key of a given type.
    ///
    /// Bumps by one the derivation index counter and generates the key
    ...
    pub async fn next_unused_spending_key(&mut self, key_type: KeyType) -> SpendingKey {
        match key_type {
            ...
            KeyType::SecretAddress => self.next_unused_secret_address_key().await.into(),
        }
    }

    /// Bump the derivation index for the given key type.
    ///
    /// Check that the index increases; so this function disallows rolling it
    /// back. Also, set the new counter to one plus `max_used_index`.
    pub async fn bump_derivation_index(&mut self, key_type: KeyType, max_used_index: u64) {
        let current_counter = self.key_counter(key_type);

        if current_counter < max_used_index + 1 {
            debug!(
                "Adding {} new {key_type} keys to wallet",
                max_used_index + 1 - current_counter
            );
            match key_type {
                KeyType::Generation => {
                    self.wallet_db
                        .set_generation_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                ...
                KeyType::Symmetric => {
                    self.wallet_db
                        .set_symmetric_key_counter(max_used_index.saturating_add(1))
                        .await;
                ...
                KeyType::EcHybrid => {
                    self.wallet_db
                        .set_ec_hybrid_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                        let key = self.wallet_entropy.nth_ec_hybrid_key(idx).into();
                        self.known_ec_hybrid_keys.push(key);
                    }
                }
                KeyType::SecretAddress => {
                    self.wallet_db
                        .set_secret_address_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                        let key = self.wallet_entropy.nth_secret_address_key(idx).into();
                        self.known_secret_address_keys.push(key);
                    }
                }
            }
        }
    ...
    /// Get current counter for the spending key of the given key type.
    pub fn key_counter(&self, key_type: KeyType) -> u64 {
        match key_type {
            ...
            KeyType::SecretAddress => self.wallet_db.get_secret_address_key_counter(),
        }
    }
    ...
    pub fn nth_spending_key(&self, key_type: KeyType, derivation_index: u64) -> SpendingKey {
        match key_type {
            ...
            KeyType::EcHybrid => self
                ...
                .into(),
            KeyType::SecretAddress => self
                .wallet_entropy
                .nth_secret_address_key(derivation_index)
                .into(),
        }
    }
    ...
    /// Also: the returned key is added to the cache of known keys.
    ///
    /// Note that incrementing the counter modifies wallet state.  It is
    /// important to write to disk afterward to avoid possible funds loss.
    async fn next_unused_generation_spending_key(
        ...
        let current_counter = self.wallet_db.get_generation_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db.set_generation_key_counter(new_counter).await;
        let key = self.wallet_entropy.nth_generation_spending_key(new_index);
    ...
    pub async fn next_unused_symmetric_key(&mut self) -> symmetric_key::SymmetricKey {
        let current_counter = self.wallet_db.get_symmetric_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db.set_symmetric_key_counter(new_counter).await;
        let key = self.wallet_entropy.nth_symmetric_key(new_index);
        self.known_symmetric_keys.push(key.into());
        key
    }

    /// Get the next unused elliptic curve hybrid key.
    ///
    /// Returns key at present counter, and increments the counter.
    /// Also: the returned key is added to the cache of known keys.
    pub async fn next_unused_ec_hybrid_key(&mut self) -> elliptic_curve_hybrid::EcHybridKey {
        let current_counter = self.wallet_db.get_ec_hybrid_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db.set_ec_hybrid_key_counter(new_counter).await;
        let key = self.wallet_entropy.nth_ec_hybrid_key(new_index);
        self.known_ec_hybrid_keys.push(key.clone().into());

        key
    }

    /// Get the next unused secret address key.
    ///
    /// Returns key at present counter, and increments the counter.
    /// Also: the returned key is added to the cache of known keys.
    pub async fn next_unused_secret_address_key(&mut self) -> secret_address::SecretAddressKey {
        let current_counter = self.wallet_db.get_secret_address_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db
            .set_secret_address_key_counter(new_counter)
            .await;
        let key = self.wallet_entropy.nth_secret_address_key(new_index);
        self.known_secret_address_keys.push(key.into());
    ...
    /// Get the next n elliptic curve hybrid keys (with derivation indices)
    /// without modifying the counter.
    pub(crate) fn get_future_ec_hybrid_keys(
        &self,
        num_future_keys: usize,
    ) -> impl Iterator<Item = (u64, elliptic_curve_hybrid::EcHybridKey)> + use<'_> {
        let index = self.wallet_db.get_ec_hybrid_key_counter();
        (index..index + (num_future_keys as u64))
            .map(|i| (i, self.wallet_entropy.nth_ec_hybrid_key(i)))
    }

    pub(crate) fn get_future_secret_address_keys(
        &self,
        num_future_keys: usize,
    ) -> impl Iterator<Item = (u64, secret_address::SecretAddressKey)> + use<'_> {
        let index = self.wallet_db.get_secret_address_key_counter();
        (index..index + (num_future_keys as u64))
            .map(|i| (i, self.wallet_entropy.nth_secret_address_key(i)))
    }
    ...
    /// If incoming UTXOs are found, the relevant key counters are updated.
    ///
    /// Only announced UTXOs actually present in the transaction are returned
    /// here, it's not sufficient that they are announced.
    async fn recover_by_scanning(&mut self, new_block: &Block) -> Vec<IncomingUtxo> {
        ...
        let mut max_derivation_indices = HashMap::<KeyType, u64>::new();
        ...
        {
            max_derivation_indices
                .entry(key_type)
                .and_modify(|current_max_derivation_index| {
                    if derivation_index > *current_max_derivation_index {
        ...
        for (key_type, derivation_index) in max_derivation_indices {
            self.bump_derivation_index(key_type, derivation_index).await;
        }
    ...
    async fn process_inputs_and_outputs_maintain_mps(
        ...
        spent_inputs: &HashMap<AbsoluteIndexSet, (Utxo, u64, u64)>,
        ...
        while let Some(removal_record) = removal_records.pop() {
            ...
            if let Some((_spent_utxo, mutxo_list_index, _aocl_leaf_index)) =
    ...
    async fn process_inputs_no_maintain_mps(
        ...
        for (_, mutxo_list_index, _) in spent_inputs.values() {
            self.wallet_db
                .mark_mutxo_as_spent_known_block(
                    *mutxo_list_index,
                    block.hash(),
    ...
    pub async fn update_wallet_state_with_new_block(
        ...
        debug!(
            "\nNum known generation keys: {};\n \
              ...
              Num known secret address keys:  {};\n \
              ",
            self.known_generation_keys.len(),
            self.known_symmetric_keys.len(),
            self.known_ec_hybrid_keys.len(),
            self.known_secret_address_keys.len()
        );
    ...
    pub(crate) async fn is_synced_to(&self, tip_hash: Digest, is_true_archival: bool) -> bool {
    ...
    /// Compute a [WalletStatus], which is a snapshot of all balance-affecting
    /// UTXOs the wallet knows about along with useful metadata.
    pub(crate) async fn get_wallet_status<'a>(
        &self,
        validator: &UtxoValidator<'a>,
    ) -> WalletStatus {
        // Get UTXOs from monitored UTXOs database.
        let mut synced_utxos = vec![];
        let mut unsynced_utxos = vec![];
        let monitored_utxos = self.wallet_db.monitored_utxos();
        let stream = monitored_utxos.stream().await;
        pin_mut!(stream); // needed for iteration
        while let Some((_i, mutxo)) = stream.next().await {
            let mutxo_status = validator.mutxo_state(&mutxo).await;
            ...
            let (_, _, confirmation_height) = confirmed_in_block;

            match mutxo_status {
                MonitoredUtxoState::SyncedAndUnspent => {
                    synced_utxos.push(SyncedUtxo::new_unspent(
                        utxo,
                        aocl_leaf_index,
                        sender_randomness,
                        receiver_preimage,
                ...
                MonitoredUtxoState::Spent(maybe_known_block_spend_height) => {
                    let spend_height =
                        maybe_known_block_spend_height.unwrap_or(confirmation_height);
                    synced_utxos.push(SyncedUtxo::new_spent(
                        utxo,
                        aocl_leaf_index,
                        sender_randomness,
                        receiver_preimage,
                ...
                MonitoredUtxoState::Unsynced => unsynced_utxos.push(UnsyncedUtxo {
                    utxo,
                    sender_randomness,
                    receiver_preimage,
                    aocl_leaf_index,
        ...
        let outgoing_mempool_utxos = self
            .mempool_outgoing_utxos_iter()
            .map(|(utxo, aocl_leaf_index)| OutgoingMempoolUtxo {
                utxo: utxo.clone(),
                aocl_leaf_index: *aocl_leaf_index,
            })
            .collect_vec();
        let incoming_mempool_utxos = self
            .mempool_incoming_utxos_iter()
            .map(
                |(utxo, sender_randomness, receiver_preimage)| IncomingMempoolUtxo {
                    utxo: utxo.clone(),
                    sender_randomness,
                    receiver_preimage,
                },
...
pub(crate) mod tests {
    ...
    async fn does_not_make_tx_with_timelocked_utxos() {
        ...
        assert!(wallet_state_genesis
            ...
            .is_zero());
        assert!(!wallet_state_genesis
            .confirmed_available_balance(block_height, released_timestamp)
            .is_zero());

        let tx_initiator: TransactionInitiator = alice.clone().into();
        assert!(
            tx_initiator
                .input_candidates(launch_timestamp)
                .await
                .is_empty(),
        ...
        assert!(
            !tx_initiator
                .input_candidates(released_timestamp)
                .await
                .is_empty(),
        ...
        let input_utxos = tx_initiator.input_candidates(block_1_timestamp).await;
        assert!(
            input_utxos
                .iter()
    ...
    #[apply(shared_tokio_runtime)]
    #[traced_test]
    async fn update_wallet_state_repeated_addition_records() {
        let network = Network::Main;
        let cli = cli_args::Args::default_with_network(network);
            ...
            .unwrap();
        {
            ...
            let wallet_status = ags.get_wallet_status_for_tip().await;
            println!(
                "got {} synced utxos of total value {}",
                wallet_status.synced_unspent(None).count(),
                wallet_status
                    .synced_unspent(None)
            ...
            assert_eq!(
                ...
                wallet_status.unconfirmed_available_balance(tx_block2.kernel.timestamp),
    ...
    async fn test_invalid_type_script_states() {
            ...
            .unwrap();
        {
            ...
            assert!(
                wallet_status
                    .confirmed_available_balance(block2.header().height, tx_block2.kernel.timestamp)
                    .is_zero(),
    ...
    mod guesser_fee_utxos {
        ...
        async fn registers_guesser_fee_utxos_correctly() {
...

            {
                ...
                assert!(
                    !wallet_status
                        .confirmed_available_balance(block1.header().height, block1_timestamp)
                        .is_positive(),
                ...
                .unwrap();
            {
                ...
                assert!(
                    !wallet_status
                        .confirmed_available_balance(block2.header().height, block2_timestamp)
                        .is_positive(),
    ...
    mod wallet_balance {
        ...
        async fn confirmed_and_unconfirmed_balance() -> Result<()> {
                ...
                .height;
            {
                ...
                assert_eq!(
                    wallet_status.unconfirmed_available_balance(timestamp),
    ...
    mod abandoned_mutxos {
        ...
        async fn mutxos_spent_in_orphaned_blocks_are_still_spendable() {
            ...
            assert_eq!(
                ...
                alice_global_lock
                    ...
                    .get_wallet_status_for_tip()
                    .await
                    .confirmed_total_balance(genesis.header().height),
            ...
            let wallet_status_1a = alice_global_lock
                ...
                .get_wallet_status_for_tip()
                .await;
            assert!(wallet_status_1a
                .confirmed_total_balance(block_1a.header().height)
                .is_zero());
            ...
            assert_eq!(
                init_balance,
                wallet_status_2b.confirmed_total_balance(block_2b.header().height),
            ...
            assert!(alice_global_lock
                ...
                .get_wallet_status_for_tip()
                .await
                .confirmed_total_balance(block_2a.header().height)
                .is_zero());
        }
        ...
        async fn abandoned_utxo_is_unsynced() {
            ...
            assert_eq!(
                ...
                wallet_status_1a.confirmed_total_balance(block_1a.header().height),
            );

            assert_eq!(wallet_status_1a.num_unsynced(), 0);
            ...
            let wallet_status_1b = alice_global_lock
                ...
                .get_wallet_status_for_tip()
                .await;
            assert!(wallet_status_1b
                .confirmed_total_balance(block_1b.header().height)
                .is_zero());
            assert_ne!(wallet_status_1b.num_unsynced(), 0);
        }
    }
    ...
    pub(crate) mod scan_mode {
        ...
        async fn test_recovery_on_imported_wallet() {
            ...
            let derivation_index = 20;
            let alice_future_spending_key =
                alice_secret.nth_generation_spending_key(derivation_index);
            ...
            for (cli_args, should_catch_utxo) in [
                ...
                let validator = UtxoValidator::Light {
                    tip: block_1.hash(),
                    tip_msa: block_1.mutator_set_accumulator_after().unwrap(),
                };
                let wallet_status_ = alice_wallet_state.get_wallet_status(&validator).await;
                let balance_ =
                    wallet_status_.confirmed_available_balance(block_1.header().height, now);
                ...
                let wallet_status = alice_wallet_state.get_wallet_status(&validator).await;
                let balance =
                    wallet_status.confirmed_available_balance(block_1.header().height, now);
        ...
        async fn get_future_keys_do_not_modify_counters() {
            ...
            let future_symmetric_keys = wallet_state
                ...
                .collect_vec();
            let future_ech_keys = wallet_state
                .get_future_ec_hybrid_keys(num_future_keys)
                .collect_vec();
            ...
            assert_eq!(
                ech_counter,
                wallet_state.wallet_db.get_ec_hybrid_key_counter(),
            );
        ...
        async fn scan_mode_recovers_unexpected_offchain_composer_utxos() {
            ...
            println!(
                ...
                wallet_status.synced_spent(None).count()
            );
            println!(
                "wallet status -- # unsynced: {}",
                wallet_status.num_unsynced()
            );
            assert_eq!(2, wallet_status.synced_unspent(None).count());
        }
    }
    ...
    pub(crate) mod fee_notifications {
        ...
        async fn wallet_recovers_upgrader_fee_utxos_given_notification_policy(
            ...
            assert_eq!(1, wallet_status.synced_unspent(None).count());

            let gobble_utxo = wallet_status.synced_unspent(None).next().unwrap().clone();
            assert_eq!(
                fee,
