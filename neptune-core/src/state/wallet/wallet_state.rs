...
pub struct WalletState {
    ...
    /// Cache of known viewing address keys generated by this wallet.
    known_viewing_address_keys: Vec<SpendingKey>,
...
impl WalletState {
    ...
    pub(crate) async fn try_new(
        ...
        let known_viewing_address_keys = (0..rusty_wallet_database
            .get_viewing_address_key_counter())
            .map(|idx| wallet_entropy.nth_viewing_address_key(idx).into())
            .collect_vec();
    ...
    fn get_future_spending_keys_internal(
        ...
        match key_type {
            ...
            KeyType::ViewingAddress => Box::new(
    ...
    /// An iterator over all known viewing address keys.
    fn get_known_viewing_address_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        self.known_viewing_address_keys.iter().cloned()
    }
        ...
        if current_counter < max_used_index + 1 {
            ...
            match key_type {
                ...
                KeyType::ViewingAddress => {
                    self.wallet_db
                        .set_viewing_address_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                        let key = self.wallet_entropy.nth_viewing_address_key(idx).into();
    ...
    /// Get the next unused viewing address key.
    ///
    /// Returns key at present counter, and increments the counter.
    /// Also: the returned key is added to the cache of known keys.
    pub async fn next_unused_viewing_address_key(&mut self) -> viewing_address::ViewingAddressKey {
        let current_counter = self.wallet_db.get_viewing_address_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db
            .set_viewing_address_key_counter(new_counter)
            .await;
        let key = self.wallet_entropy.nth_viewing_address_key(new_index);
        self.known_viewing_address_keys.push(key.into());
    ...
    pub(crate) fn get_future_viewing_address_keys(
        &self,
        num_future_keys: usize,
    ) -> impl Iterator<Item = (u64, viewing_address::ViewingAddressKey)> + use<'_> {
        let index = self.wallet_db.get_viewing_address_key_counter();
        (index..index + (num_future_keys as u64))
            .map(|i| (i, self.wallet_entropy.nth_viewing_address_key(i)))
    }
    ...
    pub async fn update_wallet_state_with_new_block(
        ...
        debug!(
            ...
            self.known_viewing_address_keys.len()
        );
    ...
    pub(crate) mod scan_mode {
        ...
        async fn scan_for_utxos_announced_to_future_keys_behaves(
            ...
            #[strategy(collection::vec(any::<bool>(), 4 * NUM_FUTURE_KEYS))] mut select_vec: Vec<
                bool,
            >,
            #[strategy(collection::vec(any::<bool>(), 4 * NUM_FUTURE_KEYS))] mut own_lock_vec: Vec<
                bool,
            >,
        ) {
            let network = Network::Main;
        ...
        async fn scan_mode_ignores_future_key_utxos_with_foreign_lock_script(
            ...
            let kernel = TransactionKernelModifier::default()
                ...
                .outputs(outputs)
                .modify(kernel);
            let block1 = invalid_block_with_tx_kernel(&genesis, kernel);
