...
impl WalletState {
    ...
    ///
    /// Since the wallet keeps track of the count, and since new generation keys
    /// are derived with the counter value as the next derivation index, we can
    /// consider generating these lazily instead of populating the cache at
    /// startup.
    ...
    ///
    /// Since the wallet keeps track of the count, and since new symmetric keys
    /// are derived with the counter value as the next derivation index, we can
    /// consider generating these lazil instead of populating the cache at
    /// startup.
