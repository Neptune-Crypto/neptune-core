...
pub struct WalletState {
    ...
    /// Cache of private address keys generated by this wallet.
    known_private_address_keys: Vec<SpendingKey>,
...
impl WalletState {
    ...
    pub(crate) async fn try_new(
        ...
        let known_private_address_keys = (0..rusty_wallet_database
            .get_private_address_key_counter())
            .map(|idx| wallet_entropy.nth_private_address_key(idx).into())
            .collect_vec();
    ...
    /// An iterator over all known symmetric spending keys.
    fn get_known_private_address_keys(&self) -> impl Iterator<Item = SpendingKey> + '_ {
        self.known_private_address_keys.iter().cloned()
    }
    ...
    pub async fn next_unused_spending_key(&mut self, key_type: KeyType) -> SpendingKey {
        match key_type {
            ...
            KeyType::Private => self.next_unused_private_address_key().await.into(),
        }
    }
    ...
    pub async fn bump_derivation_index(&mut self, key_type: KeyType, max_used_index: u64) {
        ...
        if current_counter < max_used_index + 1 {
            ...
            match key_type {
                ...
                KeyType::Private => {
                    self.wallet_db
                        .set_private_address_key_counter(max_used_index.saturating_add(1))
                        .await;

                    for idx in current_counter..=max_used_index {
                        let key = self.wallet_entropy.nth_private_address_key(idx).into();
                        self.known_private_address_keys.push(key);
                    }
                }
            }
        }
    ...
    pub fn key_counter(&self, key_type: KeyType) -> u64 {
        match key_type {
            ...
            KeyType::Private => self.wallet_db.get_private_address_key_counter(),
        }
    }
    ...
    pub fn nth_spending_key(&self, key_type: KeyType, derivation_index: u64) -> SpendingKey {
        match key_type {
            ...
            KeyType::Symmetric => self
                ...
                .into(),
            KeyType::Private => self
                .wallet_entropy
                .nth_private_address_key(derivation_index)
                .into(),
        }
    }
    ...
    /// Get the next unused private address key.
    ///
    /// Returns key at present counter, and increments the counter.
    /// Also: the returned key is added to the cache of known keys.
    pub async fn next_unused_private_address_key(&mut self) -> private_address::PrivateAddressKey {
        let current_counter = self.wallet_db.get_private_address_key_counter();
        let new_index = current_counter;
        let new_counter = current_counter.saturating_add(1);
        self.wallet_db
            .set_private_address_key_counter(new_counter)
            .await;
        let key = self.wallet_entropy.nth_private_address_key(new_index);
        self.known_private_address_keys.push(key.clone().into());
