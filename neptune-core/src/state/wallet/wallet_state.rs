...
impl WalletState {
    ...
    /// Scan the given block for UTXOs announced to any of the given keys.
    ///
    ...
    pub async fn update_wallet_state_with_new_block(
        ...
        let potential_duplicates =
            Self::potential_duplicate_utxos(monitored_utxos, &incoming).await;
    ...
    pub(crate) async fn rescan(
        ...
        let monitored_utxos = self.wallet_db.monitored_utxos_mut();
        let potential_duplicates =
            Self::potential_duplicate_utxos(monitored_utxos, &incoming_hashmap).await;
        let recovery_list = Self::process_outputs_no_maintain_mps(
            &block,
            &incoming_hashmap,
