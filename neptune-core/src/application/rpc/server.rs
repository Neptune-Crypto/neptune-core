        ...
        let maybe_prepared_mutxo = match mined_in_block {
            Some(block) => {
                ...
                let msmp = ams
                    ...
                    .map_err(|x| {
                        anyhow!(
                            "Could not restore mutator set membership proof. Is archival mutator set corrupted? Got error: {x}"
                        )
                    })?;
                ...
                if !msa.verify(item, &msmp) {
                        ...
                        .await
                    {
                        ...
                        error!(
                            "Claimed UTXO's mutator set membership proof was invalid but we could not find the block in which it was spent. This is most likely a bug in the software."
                        );
                    }
                }
                Some(monitored_utxo)
    ...
    async fn set_coinbase_distribution(
        ...
        for output in coinbase_distribution_readable {
            let output = match output.into_coinbase_output(network) {
                Ok(cbo) => cbo,
                Err(err) => {
                    return Err(RpcError::InvalidCoinbaseDistribution(err.to_string()));
                }
            };
...
mod tests {
    ...
    async fn announcements_in_block_test(
        #[strategy(
            txkernel::with_lengths(0usize, 2usize, NUM_ANNOUNCEMENTS_BLOCK1, false)
        )]
        ...
        async fn send_rate_limit() -> Result<()> {
            ...
            for i in 0..10 {
                ...
                match i {
                    ...
                    _ => {
                        assert!(matches!(result, Err(RpcError::SendError(s)) if s
                            .contains("Send rate limit reached")))
                    }
                }
            }
            Ok(())
