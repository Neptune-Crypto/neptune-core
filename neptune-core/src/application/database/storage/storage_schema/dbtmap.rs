...
/// a key/value pair is inserted, it cannot be removed again. However, the
/// whole mapping can be deleted, and existing values can be overwritten. If
/// some notion of deletion is required, the value entries must be declared such
/// that they can be marked as deleted by the caller.
#[derive(Debug)]
