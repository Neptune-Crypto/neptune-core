...
use crate::{api::export::Timestamp, application::loops::channel::{self, PeerTaskToMain}, protocol::{consensus::transaction::transaction_kernel::TransactionConfirmabilityError, peer::NegativePeerSanction}};
...
pub(crate) async fn the(
    ...
    if transaction.is_valid(network, crate::protocol::consensus::consensus_rule_set::ConsensusRuleSet::infer_from(network, current_block_height)).await {
        ...
        if transaction.kernel.coinbase.is_some() {
        ...
        } else if global_state_lock.lock_guard().await.mempool.accept_transaction(
            transaction.kernel.txid(),
            transaction.proof.proof_quality().expect("guarded by `TransferTransaction`"),
            ...
            if transaction.is_confirmable_relative_to(&mutator_set_accumulator_after) {
                ...
                if crate::protocol::consensus::block::mutator_set_update::MutatorSetUpdate::new(
                    ...
                    if transaction.kernel.timestamp < now - Timestamp::seconds(crate::state::mempool::MEMPOOL_TX_THRESHOLD_AGE_IN_SECS) {
                        ...
                        to_main.send(PeerTaskToMain::Transaction(Box::new(channel::PeerTaskToMainTransaction {
                            transaction,
                    ...
                    Some(Some(NegativePeerSanction::CannotApplyTransactionToMutatorSet))
                }
            } else {
                warn!("Received unconfirmable transaction with TXID {}. Unconfirmable because:", transaction.kernel.txid());
                // get fine-grained error code for informative logging
                Some(Some(match transaction.kernel.is_confirmable_relative_to(&mutator_set_accumulator_after).err().expect(crate::application::loops::MSG_CONDIT) {
                    TransactionConfirmabilityError::InvalidRemovalRecord(invalid) => {
                        warn!("invalid removal record (at index {invalid})");
                        debug!("absolute index set of removal record {invalid}: {:?}", transaction.kernel.inputs[invalid].absolute_indices);
            ...
            Some(None) 
            // ~~TODO check this case is at least of for `Swarm`, ignoring it doesn't sits well with @skaunov, but~~
