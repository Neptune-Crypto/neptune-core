...
pub fn peerid_split(
    ...
    if couldendwith_peerid.is_empty() {
        (None, couldendwith_peerid)
    } else if let Protocol::P2p(peerid) = couldendwith_peerid.iter().last().expect(MSG_CONDIT) {
...
/// *destructively* strips the `PeerId` endings from `madrs`, and pops one or two `Protocol`
pub fn socketaddr_tryfrom(madr: &mut Multiaddr) -> Option<SocketAddr> {
    if let Some(Protocol::Tcp(port)) = peerid_split(madr).1.pop() {
        Some(SocketAddr::new(
            match madr.pop() {
