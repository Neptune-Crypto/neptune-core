...
pub fn is_ipadr_in_madrs(
    ...
    madrs
        .filter(|madr| {
            !itertools::Itertools::contains(&mut madr.protocol_stack(), &Protocol::P2pCircuit.tag())
        })
        // .iter()
        .any(|madr| itertools::Itertools::contains(&mut madr.iter(), &ipadr.into()))
}

/// *destructive: strips the `PeerId` endings from `madrs`*
///
...
pub fn does_madrs_cover_socketadr(
    socketadr: &std::net::SocketAddr,
    madrs: std::slice::IterMut<'_, Multiaddr>,
) -> bool {
    // let socketadr = [
    ...
    // let madrs =
    madrs
        .map(|madr| super::peerid_split(madr).1)
