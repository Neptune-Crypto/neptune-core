...
fn relay_connect_ifneeded(
    ...
    let mut relays = peer_infos.iter().filter(
        |(_id, info)| info.protocols.contains(
            &libp2p::relay::HOP_PROTOCOL_NAME // TODO debug this to be sure this approach works
        ) && info.listen_addrs.iter().any(relay_maybe)
    ).filter_map(|(id, _info)| peer_pings.get(&id).map(|ping| (id, _info, ping))) // the only case @skaunov see yet when `peer_pings` can have no `&id` is `ping` `Timeout` and the peer caught disconnecting and to `remove` from infos; so it's ok to just filter him out
    ...
    if listener_added {
        ...
        tracing::trace!["we know no peers we can build a `P2pCircuit` on which"]
    }
}}