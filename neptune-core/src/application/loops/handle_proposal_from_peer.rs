...
pub(crate) async fn the(
    ...
    match proposal.body().total_guesser_reward() {
        Err(_) => Some(PeerSanction::Negative(NegativePeerSanction::InvalidBlockProposal)),
        ...
        Ok(incoming_guesser_fee) => 
            ...
            if global_state_lock.cli().accept_block_proposal_from(peer_address).is_err() {
                ...
                Some(PeerSanction::Negative(NegativePeerSanction::BlockProposalFromBlockedPeer))
            } else {
                ...
                if proposal.is_valid(state.chain.light_state(), &Timestamp::now(), global_state_lock.cli().network).await {
                    ...
                    drop(state);
                    Some(PeerSanction::Negative(NegativePeerSanction::InvalidBlockProposal))
                }
            },
    }
}