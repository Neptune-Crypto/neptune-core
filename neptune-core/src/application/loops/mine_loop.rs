...
fn guess_worker(
    ...
    // mainnet.
    let should_reset_difficulty = Block::should_reset_difficulty(
        network,
        now,
        previous_block_header.timestamp,
        previous_block_header.height,
    ); // todo hduoc: check this
    let new_difficulty = if should_reset_difficulty {
        ...
        let target_block_interval = override_target_block_interval
            .unwrap_or(network.target_block_interval(previous_block_header.height)); // todo hduoc: check this
        difficulty_control(
            now,
            previous_block_header.timestamp,
...
pub(crate) mod tests {
    ...
    async fn hash_rate_independent_of_tx_size() {
        ...
        let hash_rate_empty_tx =
            estimate_own_hash_rate(network.target_block_interval(BlockHeight::genesis()), 0).await;
        println!("hash_rate_empty_tx: {hash_rate_empty_tx}");

        let hash_rate_big_tx =
            estimate_own_hash_rate(network.target_block_interval(BlockHeight::genesis()), 10000)
                .await;
    ...
    async fn testnet_mock_reset_difficulty() -> anyhow::Result<()> {
        ...
        for i in 1..=num_blocks {
            ...
            let reset_interval = network
                .difficulty_reset_interval(BlockHeight::new((i as u64).into()))
                .unwrap();
