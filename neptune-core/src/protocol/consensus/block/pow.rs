...
impl<const MERKLE_TREE_HEIGHT: usize> GuesserBuffer<MERKLE_TREE_HEIGHT> {
    /// Return a guesser buffer that for non-memory hard guessing. Contains an
    /// empty Merkle tree since a Merkle tree is not required when the PoW
    /// algorithm is not memory-hard.
