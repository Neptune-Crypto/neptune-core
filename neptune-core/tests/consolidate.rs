...
pub async fn consolidation_tx_with_lustration() {
    ...
    let old_block_height: BlockHeight = u64::from(num_blocks_mined).into();
    assert_eq!(
        old_block_height,
    ...
    // The consolidation transaction must lustrate because UTXOs generated by
    // block 1 must lustrate. And Block 1 was mined by Alice.
    let accept_lustrations = true;
    let max_num_inputs = 3;
    let _ = alice
        .gsl
    ...
    let consensus_rule_set = ConsensusRuleSet::infer_from(Network::RegTest, old_block_height);
    let lustrated_amount = mempool_tx
        .kernel
    ...
    println!(
        "lustration_status_after.counter: {}",
        lustration_status_after.counter
    );
    println!(
        "lustration_status_before.counter: {}",
        lustration_status_before.counter
    );
    assert_eq!(
        lustration_status_before
            .counter
            .checked_sub(&lustrated_amount)
            .unwrap(),
        lustration_status_after.counter
    )
}
