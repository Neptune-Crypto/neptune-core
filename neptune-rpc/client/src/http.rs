...
mod tests {
    ...
    async fn client_responds_in_real_world_scenario() {
        ...
        let client =
            start_pseudo_real_server(HashSet::from([Namespace::Chain]), unsafe_rpc, 40500, None)
                .await;
    ...
    async fn validate_donation_address() {
        let unsafe_rpc = false;
        let client =
            start_pseudo_real_server(HashSet::from([Namespace::Wallet]), unsafe_rpc, 40550, None)
                .await;
