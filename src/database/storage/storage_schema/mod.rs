    ...
    */
    ...
    /*
        pub mod concurrency {
            ...
            mod storage_vec {
                use super::*;
                use super::super::super::super::storage_vec::traits::tests as traits_tests;

                async fn gen_concurrency_test_vec() -> TestVec {
                    // open new NeptuneLevelDb that will be removed on close.
                    let db = NeptuneLevelDb::open_new_test_database(true, None, None, None).await.unwrap();
                    let mut rusty_storage = SimpleRustyStorage::new(db);
                    rusty_storage.schema.new_vec::<u64>("atomicity-test-vector").await
                }

                #[tokio::test(flavor = "multi_thread")]
                #[should_panic]
                async fn non_atomic_set_and_get() {
                    traits_tests::concurrency::non_atomic_set_and_get(&mut gen_concurrency_test_vec().await).await;
                }
                ...
                async fn non_atomic_set_and_get_wrapped_atomic_rw() {
                    traits_tests::concurrency::non_atomic_set_and_get_wrapped_atomic_rw(
                        &mut gen_concurrency_test_vec().await,
                    ).await;
                }
                ...
                async fn atomic_setall_and_getall() {
                    traits_tests::concurrency::atomic_setall_and_getall(&mut gen_concurrency_test_vec().await).await;
                }

                // #[tokio::test(flavor = "multi_thread")]
                // async fn atomic_iter_mut_and_iter() {
                //     traits_tests::concurrency::atomic_iter_mut_and_iter(&mut gen_concurrency_test_vec()).await;
                // }
            }
            ...
            mod storage_singleton {
                ...
                async fn gen_atomic_rw_test_singleton(num: u8) -> AtomicRw<Vec<TestSingleton>> {
                    AtomicRw::from(gen_non_atomic_test_singleton(num).await)
                }

                async fn gen_non_atomic_test_singleton(num: u8) -> Vec<TestSingleton> {
                    // open new NeptuneLevelDb that will be removed on close.
                    let db = NeptuneLevelDb::open_new_test_database(true, None, None, None).await.unwrap();
                    ...
                    for _i in 0..num {
                        let mut singleton = storage.schema.new_singleton::<u64>("singleton".to_owned()).await;
                        singleton.set(25).await;
                        list.push(singleton)
                    }
                    list
                }
                ...
                #[tokio::test(flavor = "multi_thread")]
                pub async fn multi_table_atomic_rw_setmany_and_getmany() {
                    let table_singletons = gen_atomic_rw_test_singleton(2).await;

                    // performs single-line read lock on table_singletons to copy original state.
                    let orig = table_singletons.lock_guard().await[0].get();
