...
mod tests {
    ...
    fn get_persisted_vec_with_length(
        length: Index,
        name: &str,
    ) -> (OrdinaryVec<u64>, Vec<u64>, DB) {
        let mut db = get_test_db(true);
        let mut persisted_vec = OrdinaryVec::new(db.clone(), 0, name);
    ...
    fn test_simple_prop() {
        ...
        let delegated_db_vec: OrdinaryVec<[u8; 13]> =
            OrdinaryVec::new(db.clone(), 0, "unit test vec 0");
    ...
    fn multiple_vectors_in_one_db() {
        let mut db = get_test_db(true);
        let mut delegated_db_vec_a: OrdinaryVec<u128> =
            OrdinaryVec::new(db.clone(), 0, "unit test vec a");
        let mut delegated_db_vec_b: OrdinaryVec<u128> =
            OrdinaryVec::new(db.clone(), 1, "unit test vec b");
    ...
    fn db_close_and_reload() {
        ...
        let mut vec: OrdinaryVec<u128> = OrdinaryVec::new(db, 0, "vec1");
        vec.push(1000);
        ...
        let mut vec2: OrdinaryVec<u128> = OrdinaryVec::new(db2, 0, "vec1");
