...
///
/// The mempool also keeps a view of how "valuable" a transaction is, from the
/// perspective the the caller inserting the transaction. However, this value
/// is not used to determine which transactions gets to stay in the mempool in
/// the case of a full mempool, since such a value is subjective, and a goal is
/// to have different nodes running with the same mempool policy to agree on the
/// content of the mempool at any time.
///
...
pub struct Mempool {
    ...
    /// order. Only transactions with a positive transaction value are stored in
    /// this priority queue.
    // This is relatively small compared to `tx_dictionary`
    #[get_size(ignore)]
...
impl Mempool {
    ...
    pub fn new(max_total_size: ByteSize, max_num_transactions: Option<usize>, tip: &Block) -> Self {
        ...
        let tip_mutator_set_hash = tip.mutator_set_accumulator_after().hash();
    ...
    pub(super) fn set_sync_labels(&mut self, tip: &Block) {
    ...
    /// prioritize transactions with a positive value first, then transactions
    ...
    pub(super) fn remove(&mut self, transaction_id: TransactionKernelId) -> Option<MempoolEvent> {
        self.tx_dictionary.remove(&transaction_id).map(|tx| {
            ...
            self.values.remove(&transaction_id);
...
mod tests {
    ...
    pub async fn insert_then_get_then_remove_then_get() {
        ...
        mempool.insert(txs[0].clone(), NativeCurrencyAmount::zero());
        assert!(mempool.contains(transaction_digests[0]));
        assert!(!mempool.contains(transaction_digests[1]));
    ...
    fn setup_mock_mempool(transactions_count: usize, sync_block: &Block) -> Mempool {
        ...
        for mut tx in txs {
            tx.kernel = TransactionKernelModifier::default()
                ...
                .modify(tx.kernel);
            mempool.insert(tx, NativeCurrencyAmount::zero());
        }
    ...
    async fn most_dense_proof_collection_test() {
        ...
        mempool.insert(tx_by_bob.into(), NativeCurrencyAmount::zero());
        assert_eq!(
            mempool
                .preferred_proof_collection(bob.cli.max_num_proofs)
                .unwrap()
    ...
    fn only_txs_with_up_to_date_mutator_set_hashes_are_returned_for_block_inclusion() {
        ...
        for i in 0..5 {
            ...
            for tx in &mut txs {
                ...
                mempool.insert(tx.to_owned(), NativeCurrencyAmount::zero());
            }
            assert_eq!(
                i,
                mempool
    ...
    async fn remove_transactions_with_block_test() {
        ...
        mempool.insert(tx_by_bob.clone(), NativeCurrencyAmount::zero());
        ...
        let tx_from_alice_original = alice
            ...
            .unwrap()
            .transaction;
        mempool.insert(tx_from_alice_original.into(), send_amount);
    ...
    async fn merged_tx_kicks_out_merge_inputs() {
        ...
        mempool.insert(left, NativeCurrencyAmount::zero());
        mempool.insert(right, NativeCurrencyAmount::zero());
        assert_eq!(2, mempool.len());
        ...
        mempool.insert(merged.clone(), NativeCurrencyAmount::zero());
        assert_eq!(1, mempool.len());
    ...
    async fn reorganization_does_not_crash_mempool() {
        ...
        alice
            ...
            .await
            .mempool
            .insert(never_mined_tx.into(), send_amt);
    ...
    async fn conflicting_txs_preserve_highest_fee() {
        ...
        .transaction;
        {
            ...
            mempool.insert(tx_low_fee.clone().into(), NativeCurrencyAmount::zero());
            assert_eq!(1, mempool.len());
        ...
        {
            let mempool = &mut preminer.lock_guard_mut().await.mempool;
            mempool.insert(tx_high_fee.clone().into(), NativeCurrencyAmount::zero());
            assert_eq!(1, mempool.len());
            assert_eq!(
                *tx_high_fee,
        ...
        // does *not* replace the existing transaction.
        {
            ...
            .transaction;
            let mempool = &mut preminer.lock_guard_mut().await.mempool;
            mempool.insert(tx_medium_fee.clone().into(), NativeCurrencyAmount::zero());
            assert_eq!(1, mempool.len());
            assert_eq!(
                *tx_high_fee,
                *mempool.get(tx_high_fee.kernel.txid()).unwrap()
    ...
    mod proof_quality_tests {
        ...
        async fn always_preserve_primitive_witness_if_available() {
            ...
            mempool.insert(pw_tx.into(), NativeCurrencyAmount::zero());

            let pc_tx =
                tx_with_proof_type(TxProvingCapability::ProofCollection, network, fee).await;
            mempool.insert(pc_tx.into(), NativeCurrencyAmount::zero());
            assert_eq!(
                1,
                mempool.len(),
            ...
            mempool.insert(sp_tx.into(), NativeCurrencyAmount::zero());
            assert_eq!(
                1,
                mempool.len(),
                "assumption: original transaction replaced"
        ...
        async fn single_proof_always_replaces_primitive_witness() {
            ...
            let genesis_block = Block::genesis(network);
            let mut mempool = setup_mock_mempool(0, &genesis_block);
            mempool.insert(pw_high_fee.into(), NativeCurrencyAmount::zero());
            ...
            mempool.insert(sp_low_fee.into(), NativeCurrencyAmount::zero());
            assert!(
                mempool.len().is_one(),
        ...
        async fn single_proof_always_replaces_proof_collection() {
            ...
            let genesis_block = Block::genesis(network);
            let mut mempool = setup_mock_mempool(0, &genesis_block);
            mempool.insert(pc_high_fee.into(), NativeCurrencyAmount::zero());
            assert!(mempool.len().is_one(), "One tx after insertion");
        ...
        async fn ms_updated_transaction_always_replaces_progenitor(
            ...
            mempool.insert(original_tx.clone(), tx_value);
            ...
            mempool.insert(updated_tx.clone(), tx_value);
