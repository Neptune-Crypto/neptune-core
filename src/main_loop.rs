...
impl MainLoopHandler {
    ...
    ///
    /// Never disconnects from peers listed as CLI arguments.
    ///
    /// Locking:
    ...
    async fn prune_peers(&self) -> Result<()> {
        ...
        // Bootstrap nodes allow a surplus of incoming connections to provide
        // their service more reliably. Now, it's time to disconnect from some
        // peers the user didn't request a connection to.
        warn!(
            "Max peer parameter is exceeded. max is {max_num_peers} but we are connected \
             to {num_peers}. Attempting to fix."
        );

        // pick peers not specified in the command line arguments to
        // disconnect from
...
mod test {
    ...
    mod peer_discovery {
        ...
        async fn no_warning_on_peer_exceeding_limit_if_connections_are_outgoing() {
            ...
            // Reject incoming connections and all outgoing peer-discovery-
            ...
            // no warnings allowed when incoming connections are rejected
            assert!(!logs_contain("WARN"));
            assert!(!logs_contain("Max peer"));

            // don't perform peer discovery when `max_num_peers` = 0
